#This program was written by Grayson Rechsteiner
#This program runs a chess game with sound effects, graphics, hints, and a fully
#functioning AI with multiple difficulties
...
LIMECOLOUR = (50 ,205, 50)
GRAYCOLOUR = (169, 169, 169)
WHITETURNCOLOUR = (245,245,220)
GREENCOLOUR = (0,100,0)
AQUACOLOUR = (152,245,255)
REDCOLOUR = (255,0,0)

#Colours for buttons
DARKGRAYCOLOUR = (128,138,135)
LIGHTGRAYCOLOUR = (224,238,238)
MENUGRAYCOLOUR = (193,205,205)

DARKBEIGECOLOUR = (205,183,158)
LIGHTBEIGECOLOUR = (245,245,220)
MENUBEIGECOLOUR = (255,228,196)

#Set up colour of piece constants
BLACK = 0
WHITE = 1

#Set up the piece constants
ROOK = 0
KNIGHT = 1
BISHOP = 2
QUEEN = 3
...
def opposite_colour(colour):
    """Returns the opposite colour of colour inputted"""
...
def draw_border_lines(windowSurface, top, bottom, left, right, lightcolour, darkcolour):                      
    """Draws line around rectangle for 3d effect"""
...
def display_instructions(windowSurface, homeicon, hinticon, soundicon, menuhomeicon):
    ...
    #Draw title
    drawTextMiddle("How To Play", titleFont, windowSurface, windowSurface.get_rect().centerx, 5, BLACKCOLOUR)
    ...
    drawText("Buttons:", headerFont, windowSurface, 100, 150, BLACKCOLOUR)
    drawText("Returns to main menu", basicFont, windowSurface, 140, 200, BLACKCOLOUR)
    drawText("Toggles sound effects on/off", basicFont, windowSurface, 140, 240, BLACKCOLOUR)
    drawText("Displays a hint", basicFont, windowSurface, 140, 280, BLACKCOLOUR)

    #Display controls text
    drawText("Controls:", headerFont, windowSurface, 100, 350, BLACKCOLOUR)
    drawText("- All user input is through clicks", basicFont, windowSurface, 100, 400, BLACKCOLOUR)
    drawText("- Click on piece to select or unselect it", basicFont, windowSurface, 100, 440, BLACKCOLOUR)
    drawText("- Legal moves will be displayed with green circles", basicFont, windowSurface, 100, 480, BLACKCOLOUR)
    drawText("- Click on square you wish to move piece to", basicFont, windowSurface, 100, 520, BLACKCOLOUR)
...
def display_menu(windowSurface, bigbishop, bigknight):
    ...
    #Draw title
    drawTextMiddle("Chess", titleFont, windowSurface, windowSurface.get_rect().centerx, 1, BLACKCOLOUR)

    #Draw rectangles to be clicked on                                     
    pygame.draw.rect(windowSurface, MENUGRAYCOLOUR, (windowSurface.get_rect().centerx - 125, 215, 250, 60))                                      
    pygame.draw.rect(windowSurface, MENUGRAYCOLOUR, (windowSurface.get_rect().centerx - 125, 285, 250, 60))
    pygame.draw.rect(windowSurface, MENUGRAYCOLOUR, (windowSurface.get_rect().centerx - 125, 355, 250, 60))                                      
    pygame.draw.rect(windowSurface, MENUGRAYCOLOUR, (windowSurface.get_rect().centerx - 125, 425, 250, 60))
    
    #Draw options to click on
    drawTextMiddle("Single Player", basicFont, windowSurface, windowSurface.get_rect().centerx, 220, BLACKCOLOUR)
    drawTextMiddle("Two Player", basicFont, windowSurface, windowSurface.get_rect().centerx, 290, BLACKCOLOUR)
    drawTextMiddle("How To Play", basicFont, windowSurface, windowSurface.get_rect().centerx, 360, BLACKCOLOUR)
    drawTextMiddle("Exit", basicFont, windowSurface, windowSurface.get_rect().centerx, 430, BLACKCOLOUR)
    ...
    draw_border_lines(windowSurface, 215, 274, windowSurface.get_rect().centerx - 125, windowSurface.get_rect().centerx + 124,
                      LIGHTGRAYCOLOUR, DARKGRAYCOLOUR)
...
def display_AI_options(windowSurface):
    ...
    #Draw rectangles to be clicked on                                     
    pygame.draw.rect(windowSurface, MENUBEIGECOLOUR, (windowSurface.get_rect().centerx - 125, 285, 250, 60))
    pygame.draw.rect(windowSurface, MENUBEIGECOLOUR, (windowSurface.get_rect().centerx - 125, 355, 250, 60))                                      
    pygame.draw.rect(windowSurface, MENUBEIGECOLOUR, (windowSurface.get_rect().centerx - 125, 425, 250, 60))
    
    #Draw options to click on
    drawTextMiddle("Easy", basicFont, windowSurface, windowSurface.get_rect().centerx, 290, BLACKCOLOUR)
    drawTextMiddle("Medium", basicFont, windowSurface, windowSurface.get_rect().centerx, 360, BLACKCOLOUR)
    drawTextMiddle("Hard", basicFont, windowSurface, windowSurface.get_rect().centerx, 430, BLACKCOLOUR)

    #Draw lines around rectangles for 3d effect
    draw_border_lines(windowSurface, 285, 344, windowSurface.get_rect().centerx - 125,
                      windowSurface.get_rect().centerx + 124, LIGHTBEIGECOLOUR, DARKBEIGECOLOUR)
...
class Board():
    ...
    def __init__(self, pieces):
        ...
        self.board = create_array(8, 8, None)                      #Create the board
        self.moved = create_array(8, 8, False)                     #Create array that keeps track of what pieces have moved
    ...
    def make_copy(self):
        """Make and return a copy of the board class"""

        board_copy = Board(self.pieces)      #Make a new board class (copy)
...
class Game():
    ...
    def __init__(self, AI, difficulty):
        ...
        #Create pieces list
        self.pieces = create_array(2, 6, None)

        #Create graveyard list (captures)
        self.graveyard = create_array(2, 1, None)
    ...
    def display_frame(self, windowSurface):
        ...
        pygame.draw.rect(windowSurface, BLACKCOLOUR, (self.surfaceboard.rect.left-5,
        ...
        pygame.draw.rect(windowSurface, WHITECOLOUR, (self.surfaceboard.rect.left,
        ...
        for i in range(len(scoretext)):
            drawText(scoretext[i], basicFont, windowSurface, 45, scoreheight[i], BLACKCOLOUR)

        #Display the captured captions
        capturedtextheight = [45, 350]
        for i in range(len(capturedtextheight)):
            drawText("Captured:", basicFont, windowSurface, 675, capturedtextheight[i], BLACKCOLOUR)
        ...
        if self.hint != ((-1, -1), (-1, -1)):
            ...
            #Draw lines for box
            self.draw_box_around_piece(windowSurface, left, right, top, bottom, REDCOLOUR)
        ...
        if self.firstslot != (-1, -1):
            ...
            if self.hint != ((-1, -1), (-1, -1)):
                ...
                if neighbour == ONRIGHT:
                    right -= 1
                elif neighbour == ONLEFT:
                    left += 1
                    
            #Draw lines for box
            self.draw_box_around_piece(windowSurface, left, right, top, bottom, LIMECOLOUR)
        ...
        for row in range(SIZE):
            for col in range(SIZE):
                ...
                if slot in self.validmoves:
                    ...
                    gfxdraw.aacircle(windowSurface, int(width), int(height), 23, LIMECOLOUR)
                    gfxdraw.filled_circle(windowSurface, int(width), int(height), 23, LIMECOLOUR)
        ...
        if not self.game_over:
            #Blacks turn
            if self.turn == BLACK:
                pygame.draw.rect(windowSurface, GREENCOLOUR, (8, 73, 29, 29))
                pygame.draw.rect(windowSurface, BLACKCOLOUR, (11, 76, 23, 23))
            #Whites turn                                    
            else:
                pygame.draw.rect(windowSurface, GREENCOLOUR, (8, 598, 29, 29))
                pygame.draw.rect(windowSurface, WHITETURNCOLOUR, (11, 601, 23, 23))
            
        #Display if black is in check
        if self.board.incheck[BLACK] and not self.win[WHITE]:
            drawText("Black In Check", basicFont, windowSurface, 452, 5, BLACKCOLOUR)

        #Display if white is in check
        elif self.board.incheck[WHITE] and not self.win[BLACK]:
            drawText("White In Check", basicFont, windowSurface, 451, 660, BLACKCOLOUR)
    ...
    def draw_win_or_tie_box(self, windowSurface, words, winFont, doneFont, left):
        ...
        pygame.draw.rect(windowSurface, BLACKCOLOUR, (245, 290, 210, 117))
        pygame.draw.rect(windowSurface, GRAYCOLOUR, (250, 295, 200, 107))
        drawText(words, winFont, windowSurface, left, 305, BLACKCOLOUR)

        #Draw "DONE" button
        pygame.draw.rect(windowSurface, MENUGRAYCOLOUR, (290, 353, 120, 35))
        drawTextMiddle("CONTINUE", doneFont, windowSurface, 350, 358, BLACKCOLOUR)           
    ...
    def display_pawn_options(self, colour, pawnimages, windowSurface):
        ...
        pygame.draw.rect(windowSurface, BLACKCOLOUR, (195, 300, 310, 100))
        #Display backgroudn
        pygame.draw.rect(windowSurface, GRAYCOLOUR, (200, 305, 300, 90))
    ...
    def check_if_pawns_at_end(self, windowSurface, side, colour):
        ...
        row = side  #Just easier to understand while writing code
